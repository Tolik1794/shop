...
class PaymentController extends AbstractAdvancedController
{
	...
	): Response
	{
		...
		return new Response($this->paymentDocumentRenderer->renderOrder($order, $store, $request));
	}
	...
	): Response
	{
		...
		return new Response($this->paymentDocumentRenderer->renderPurchase($purchase, $store, $request));
	}
	...
	private function prefillRequestedDocument(Payment $payment, Store $store, Request $request): void
	{
		if ($orderId = $request->query->getInt('order_id')) {
			...
			if ($order instanceof Order) {
				if ($request->query->get('order_payment_action') === PaymentDocumentRenderer::ORDER_PAYMENT_ACTION_REFUND) {
