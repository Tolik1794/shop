...
class InventoryCustomerReturnOperationType extends AbstractType
{
	...
	{
		$builder
			->add('orderEntry', EntityType::class, [
				'class' => OrderEntry::class,
				'query_builder' => fn (OrderEntryRepository $repository) => $options['store'] instanceof Store
					? $repository->findReturnableByStoreQB($options['store'])
					: $repository->createQueryBuilder('orderEntry')->andWhere('1 = 0'),
				'choice_label' => fn (OrderEntry $entry): string => sprintf(
					'%s · %s (%s) · %s %s',
					$entry->getOrder()?->getNumber() ?? ('#' . $entry->getOrder()?->getId()),
					$entry->getProductNameSnapshot(),
					$entry->getProductCodeSnapshot(),
					$this->translator->trans('Available'),
					$this->remaining($entry->getShippedQuantity(), $entry->getReturnedQuantity())
				),
				'placeholder' => 'Select shipped order line',
				'attr' => ['class' => 'select2'],
				'constraints' => [new NotBlank(['message' => 'Select order line.'])],
			])
			->add('reason', EntityType::class, [
				...
				'required' => false,
				'attr' => ['class' => 'select2'],
				'help' => 'Optional for customer returns.',
			])
	...
	public function configureOptions(OptionsResolver $resolver): void
	{
		$resolver->setDefaults([
			'data_class' => InventoryCustomerReturnOperation::class,
			'store' => null,
		]);
